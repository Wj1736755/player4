...
class Config(context: Context) : BaseConfig(context) {
    ...
    var activePlaylistPlayed: Int?
        get() = prefs.getInt(ACTIVE_PLAYLIST_PLAYED, -1).takeIf { it != -1 }
        set(playlistId) = prefs.edit().putInt(ACTIVE_PLAYLIST_PLAYED, playlistId ?: -1).apply()

    var autoQueueEnabled: Boolean
        get() = prefs.getBoolean(AUTO_QUEUE_ENABLED, false)
        set(enabled) = prefs.edit().putBoolean(AUTO_QUEUE_ENABLED, enabled).apply()

    var autoQueuePlaylistId: Int?
        get() = prefs.getInt(AUTO_QUEUE_PLAYLIST_ID, -1).takeIf { it != -1 }
        set(playlistId) = prefs.edit().putInt(AUTO_QUEUE_PLAYLIST_ID, playlistId ?: -1).apply()

    var autoQueueIntervalMinutes: Int
        get() = prefs.getInt(AUTO_QUEUE_INTERVAL_MINUTES, 15)
        set(minutes) = prefs.edit().putInt(AUTO_QUEUE_INTERVAL_MINUTES, minutes).apply()

    var autoQueueLastAddedTimestamp: Long
        get() = prefs.getLong(AUTO_QUEUE_LAST_ADDED_TIMESTAMP, 0)
        set(timestamp) = prefs.edit().putLong(AUTO_QUEUE_LAST_ADDED_TIMESTAMP, timestamp).apply()

    var autoQueueTrackToRemoveNext: UUID?
        get() = prefs.getString(AUTO_QUEUE_TRACK_TO_REMOVE_NEXT, null)?.let { 
            try { UUID.fromString(it) } catch (e: Exception) { null }
        }
        set(trackGuid) = prefs.edit().putString(AUTO_QUEUE_TRACK_TO_REMOVE_NEXT, trackGuid?.toString()).apply()
    ...
    var textSorting: Int
        get() = prefs.getInt(TEXT_SORTING, PLAYER_SORT_BY_DATE_ADDED or org.fossify.commons.helpers.SORT_DESCENDING)
        set(textSorting) = prefs.edit().putInt(TEXT_SORTING, textSorting).apply()
    ...
    var lastInstalledBuildNumber: Int
        get() = prefs.getInt(LAST_INSTALLED_BUILD_NUMBER, 0)
        set(buildNumber) = prefs.edit().putInt(LAST_INSTALLED_BUILD_NUMBER, buildNumber).apply()
