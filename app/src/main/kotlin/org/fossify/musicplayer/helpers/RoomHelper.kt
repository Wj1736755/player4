...
class RoomHelper(val context: Context) {
    fun insertTracksWithPlaylist(tracks: ArrayList<Track>, playlistId: Int = ALL_TRACKS_PLAYLIST_ID) {
        ...
        if (playlistId != 0 && playlistId != ALL_TRACKS_PLAYLIST_ID) {
            ...
            // Log warning for tracks without GUID
            if (withoutGuid.isNotEmpty()) {
                android.util.Log.w("RoomHelper", "⚠️ ${withoutGuid.size} tracks without GUID cannot be added to custom playlist: ${withoutGuid.map { it.title }}")
            }
            
            // Add tracks with GUID to junction table
            if (withGuid.isNotEmpty()) {
                val playlistTracks = withGuid.mapIndexed { index, track ->
                    org.fossify.musicplayer.models.PlaylistTrack(
                        playlistId = playlistId,
                        trackGuid = track.guid!!,
                        position = index
                    )
                }
                context.playlistTracksDAO.insertAll(playlistTracks)
                android.util.Log.d("RoomHelper", "✅ Added ${playlistTracks.size} tracks to playlist $playlistId")
            }
        }
