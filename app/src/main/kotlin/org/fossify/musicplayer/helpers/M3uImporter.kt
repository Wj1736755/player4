...
class M3uImporter(
    ...
    fun importPlaylist(path: String, playListId: Int) {
        ...
        try {
            ...
            // Get all tracks (track can be on multiple playlists with junction table)
            ...
            for (m3uEntry in m3uEntries) {
                for (track in existingTracks) {
                    if (m3uEntry.location.toString() == track.path || m3uEntry.title == track.title) {
                        val copy = track.copy(id = 0)
                        playlistItems.add(copy)
                    }
                }
            }

            RoomHelper(activity).insertTracksWithPlaylist(playlistItems as ArrayList<Track>, playListId)
