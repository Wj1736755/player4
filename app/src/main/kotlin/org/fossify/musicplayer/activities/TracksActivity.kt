...
class TracksActivity : SimpleMusicActivity() {
    ...
    private var showTranscriptionMode = false  // Temporary, not saved
    ...
    private fun setupOptionsMenu() {
        ...
        binding.tracksToolbar.setOnMenuItemClickListener { menuItem ->
            when (menuItem.itemId) {
                ...
                R.id.show_transcription -> {
                    showTranscriptionMode = !showTranscriptionMode
                    menuItem.isChecked = showTranscriptionMode
                    refreshTracks()
                }
        ...
        ensureBackgroundThread {
            ...
            runOnUiThread {
                if (sourceType == TYPE_ALBUM) {
                    ...
                    val currAdapter = binding.tracksList.adapter
                    if (currAdapter == null || (currAdapter as? TracksAdapter)?.showTranscription != showTranscriptionMode) {
                        // Create new adapter if it doesn't exist or transcription mode changed
                        TracksAdapter(
                            activity = this,
                            ...
                            playlist = playlist,
                            items = tracks,
                            showTranscription = showTranscriptionMode
                        ) {
                            itemClicked(it as Track)
                        }.apply {
                            binding.tracksList.adapter = this
    ...
    private fun addTrackFromPath(path: String, rescanWrongPath: Boolean) {
        ...
        if (mediaStoreId == 0L) {
            ...
            if (track != null) {
                track.id = 0
                RoomHelper(this).insertTracksWithPlaylist(arrayListOf(track), playlist!!.id)
                refreshPlaylist()
            }
        }
    }
        ...
        FilePickerDialog(this, pickFile = false, enforceStorageRestrictions = false) {
            ensureBackgroundThread {
                getFolderTracks(it, true) { tracks ->
                    RoomHelper(this).insertTracksWithPlaylist(tracks, playlist!!.id)
                    refreshPlaylist()
                }
            }
        }
    ...
    private fun itemClicked(track: Track) {
        ...
        handleNotificationPermission { granted ->
            if (granted) {
                // Track which playlist is currently being played (null if from folder/album/genre)
                config.activePlaylistPlayed = if (sourceType == TYPE_PLAYLIST) playlist?.id else null
