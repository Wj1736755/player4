...
class QueueAdapter(
    ...
    fun updateCurrentTrack() {
        context.withPlayer {
            ...
            if (track != null) {
                val lastTrackGuid = currentTrack?.guid
                currentTrack = track
                val previousIndex = items.indexOfFirst { it.guid == lastTrackGuid }
                val newIndex = items.indexOfFirst { it.guid == track.guid }
    ...
    private fun setupView(view: View, track: Track, holder: ViewHolder) {
        ItemTrackQueueBinding.bind(view).apply {
            ...
            // Configure title display based on mode
            if (showTranscription && !track.transcription.isNullOrEmpty()) {
                trackQueueTitle.maxLines = 3
                trackQueueTitle.textSize = 14f
            } else {
                trackQueueTitle.maxLines = 1
                trackQueueTitle.textSize = 16f
            }
            
            trackQueueTitle.text = if (textToHighlight.isEmpty()) displayTitle else displayTitle.highlightTextPart(textToHighlight, properPrimaryColor)

            arrayOf(trackQueueTitle, trackQueueDuration).forEach {
                val color = if (track.guid == currentTrack?.guid) {
