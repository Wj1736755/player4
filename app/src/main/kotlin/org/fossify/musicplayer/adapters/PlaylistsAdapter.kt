...
class PlaylistsAdapter(
    ...
    private val playlistDurations = HashMap<Int, Int>()  // playlistId -> totalDurationSeconds
    
    fun setPlaylistDuration(playlistId: Int, durationSeconds: Int) {
        playlistDurations[playlistId] = durationSeconds
    }
    ...
    private fun setupView(view: View, playlist: Playlist) {
        ItemPlaylistBinding.bind(view).apply {
            ...
            // Normalny czas (1:1)
            val totalDuration = playlistDurations[playlist.id] ?: 0
            playlistDuration.text = totalDuration.getFormattedDuration()
            playlistDuration.setTextColor(textColor)

            // Żółty czas (at speed) - tylko gdy speed != 1.0
            val playbackSpeed = context.config.playbackSpeed
            if (playbackSpeed != 1f && totalDuration > 0) {
                val adjustedDuration = (totalDuration / playbackSpeed).toInt()
                playlistDurationAtSpeed.text = adjustedDuration.getFormattedDuration()
                playlistDurationAtSpeed.beVisible()
            } else {
                playlistDurationAtSpeed.beGone()
            }

            playlistTracks.text = playlist.trackCount.toString()
