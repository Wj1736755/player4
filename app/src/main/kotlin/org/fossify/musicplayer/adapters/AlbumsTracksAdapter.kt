    ...
    private fun askConfirmDelete() {
        ConfirmationDialog(context) {
            ensureBackgroundThread {
                ...
                positions += selectedTracks.mapNotNull { track -> items.indexOfFirstOrNull { it is Track && it.guid == track.guid } }
    ...
    private fun displayEditDialog() {
        getSelectedTracks().firstOrNull()?.let { selectedTrack ->
            EditDialog(context as SimpleActivity, selectedTrack) { track ->
                val trackIndex = items.indexOfFirst { (it as? Track)?.guid == track.guid }
                if (trackIndex != -1) {
