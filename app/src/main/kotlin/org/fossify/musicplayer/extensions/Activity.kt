...
fun Activity.addTracksToPlaylist(tracks: List<Track>, callback: () -> Unit) {
    SelectPlaylistDialog(this) { playlistId ->
        android.util.Log.d("Activity", "🎵 Adding ${tracks.size} tracks to playlist $playlistId")
        val tracksToAdd = ArrayList<Track>()
        tracks.forEach {
            it.id = 0
            tracksToAdd.add(it)
            android.util.Log.d("Activity", "  Track: ${it.title}, GUID: ${it.guid}")
        }

        ensureBackgroundThread {
            android.util.Log.d("Activity", "🔄 Calling insertTracksWithPlaylist for ${tracksToAdd.size} tracks")
            RoomHelper(this).insertTracksWithPlaylist(tracksToAdd, playlistId)

            runOnUiThread {
                android.util.Log.d("Activity", "✅ insertTracksWithPlaylist completed, calling callback")
                callback()
            }
        }
    }
